...
import shouryaImage from "../assets/shourya.jpeg";
import {
  developerName,
  developerTitle,
  developerIntro,
    ...
    {developerName.split("").map((char, index) => (
      ...
      >
        ...
        >
          <a
            href="#projects"
            className="px-6 py-3.5 rounded-lg bg-cyber-primary text-cyber-on-primary font-display text-xs font-bold tracking-widest hover:shadow-[0_0_25px_rgba(168,232,255,0.45)] transition-all duration-300 flex items-center gap-2 group"
          >
            VIEW PROJECTS
          ...
          <a
            href="#contact"
            className="px-6 py-3.5 rounded-lg border border-cyber-primary/40 text-cyber-primary bg-transparent font-display text-xs font-bold tracking-widest hover:bg-cyber-primary/10 transition-all duration-300 flex items-center gap-2"
          >
            GET IN TOUCH
          </a>
        </motion.div>
      ...
      <div className="flex-1 mt-12 lg:mt-0 relative flex justify-center items-center">
        ...
        >
          {/* Glow Effect */}
          <div className="absolute inset-0 bg-cyber-primary/20 blur-3xl rounded-full scale-95 opacity-55 animate-pulse" />

          <motion.div
            whileHover={{ y: -8, rotate: 1 }}
            transition={{ type: "spring", stiffness: 300, damping: 20 }}
            className="relative z-10 glass-panel p-3 rounded-[2.5rem] border border-white/10 shadow-[0_15px_50px_rgba(0,0,0,0.7)]"
          >
            ...
            {/* Top Badge */}
            <div className="absolute -top-4 -right-4 glass-panel px-3 py-2 rounded-xl flex items-center gap-2 border border-cyber-primary/30 shadow-[0_8px_20px_rgba(0,0,0,0.5)]">
              <Terminal className="w-4 h-4 text-cyber-primary" />
              <span className="font-mono text-[10px] uppercase font-semibold text-cyber-primary tracking-widest">
                Full_Stack_Dev
              </span>
            </div>

            {/* Bottom Badge */}
            <div className="absolute -bottom-4 -left-4 glass-panel px-4 py-2.5 rounded-xl flex items-center gap-2 border border-cyber-secondary/30 shadow-[0_8px_20px_rgba(0,0,0,0.5)]">
              <Code className="w-4 h-4 text-cyber-secondary" />
              <span className="font-mono text-[10px] uppercase font-semibold text-cyber-secondary tracking-widest">
                NodeJS // React
              </span>
            </div>
          </motion.div>
